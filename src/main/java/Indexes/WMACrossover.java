...
public class  WMACrossover extends Indicator implements IStockIndicator {
    ...
    {
        ArrayList<Double> short_WMA = new WMA(short_period, close_price).calculate();
        ArrayList<Double> long_WMA = new WMA(long_period, close_price).calculate();
        ...
        boolean intersect;
        for (int i=long_WMA.size()-1;i>0;i--) {
            intersect= Line2D.linesIntersect(i-1,long_WMA.get(i-1),i,long_WMA.get(i),i-1,short_WMA.get(i-1+diff_period),i,short_WMA.get(i+diff_period));
            if(intersect)
            {
                diff = short_WMA.get(i+diff_period)-long_WMA.get(i);
                diffprev= short_WMA.get(i-1+diff_period)-long_WMA.get(i-1);

                if(i==long_WMA.size()-1) openprice=0; //when signal appear in last day we can't take open price from future ;d
